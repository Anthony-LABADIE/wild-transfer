...
import { IInteractionToCreateService, IInteractionToUpdateService, Message } from './service';
...
export default class InteractionService {
    ...
    async getInteractionByAuthorIdAndCommentId(authorId: string, commentId: string): Promise<Interaction | null> {
    ...
    async createInteraction({ emoji, author, comment, file }: IInteractionToCreateService): Promise<Interaction | Message> {
        try {
    ...
    async updateInteraction({
        ...
        if (interaction.author.id === author?.id && interaction.emoji.id === emoji?.id && interaction.comment?.id === comment?.id) {
