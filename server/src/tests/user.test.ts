...
describe('User resolver', () => {
    beforeAll(async () => {
        ...
        jest.spyOn(dataSource, 'getRepository').mockReturnValue(testDataSource.getRepository(User));
    });
    ...
    it('throws an error if email already exists', async () => {
        ...
        assert(res.body.kind === 'single');
        expect(res.body.singleResult.data?.CreateUser.message).toMatch(`User with email: ${testUserData.email} already exists`);
    });
    ...
    it('throws an error if username already exists', async () => {
        ...
        assert(res.body.kind === 'single');
        expect(res.body.singleResult.data?.CreateUser.message).toMatch(`User with username: ${testUserData.username} already exists`);
    });
});
