...
import { Interaction, InteractionToCreate, InteractionToUpdate } from '../entities/Interaction.entity';
...
export class InteractionResolver {
    ...
    @Query(() => Interaction)
    async GetInteractionByAuthorIdAndCommentId(@Arg('authorId') authorId: string, @Arg('commentId') commentId: string): Promise<Interaction | null> {
        return await new InteractionService().getInteractionByAuthorIdAndCommentId(authorId, commentId);
    }

    @Query(() => [Interaction])
    async GetInteractionByCommentId(@Arg('commentId') commentId: string): Promise<Interaction[]> {
    ...
    async CreateInteraction(
        ...
        const comment = commentId ? await new CommentService().getCommentById(commentId) : undefined;

        const file = fileId ? await new FileService().getFileById(fileId) : undefined;

        const emoji = emojiId ? await new EmojiService().getEmojiById(emojiId) : undefined;

        if (author && authorId && commentId && emoji) {
            const interaction = await new InteractionService().getInteractionByAuthorIdAndCommentId(authorId, commentId);
