...
import { Comment, CommentToCreate, CommentToUpdate } from '../entities/Comment.entity';
...
export class CommentResolver {
    ...
    @Mutation(() => Comment)
    async createComment(@Arg('commentToCreate') commentToCreate: CommentToCreate): Promise<Comment | MessageGql> {
        const { content, authorId, fileId, sharedUrlId } = commentToCreate;
        const author = await new UserService().getUserById(authorId);
        const file = (fileId && (await new FileService().getFileById(fileId))) || {};
        const sharedUrl = (sharedUrlId && (await new SharedUrlService().getSharedUrlById(sharedUrlId))) || {};
    ...
    @Mutation(() => MessageGql)
    async updateComment(@Arg('id') id: string, @Arg('commentToUpdate') commentToUpdate: CommentToUpdate): Promise<MessageGql> {
