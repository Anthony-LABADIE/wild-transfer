...
import { UserSharedUrl, UserSharedUrlToCreate } from '../entities/UserSharedUrl.entity';
...
const UnionMessageUserSharedUrl = createUnionType({
    ...
    resolveType: (value) => {
        return (value as UserSharedUrl).id !== undefined ? UserSharedUrl : MessageGql;
    },
});
...
export class UserSharedUrlResolver {
    ...
    @Query(() => UserSharedUrl)
    async UserSharedUrlById(@Arg('id') id: string): Promise<UserSharedUrl | undefined> {
        return await new UserSharedUrlService().getUserSharedUrlById(id);
    }

    @Query(() => [UserSharedUrl])
    async UserSharedUrlByUserId(@Arg('userId') userId: string): Promise<UserSharedUrl[]> {
        return await new UserSharedUrlService().getUserSharedUrlsByUserId(userId);
    }

    @Query(() => [UserSharedUrl])
    async UserSharedUrlByEmail(@Arg('email') email: string): Promise<UserSharedUrl[]> {
        return await new UserSharedUrlService().getUserSharedUrlsByEmail(email);
    }

    @Mutation(() => UnionMessageUserSharedUrl)
    async createUserSharedUrl(@Arg('userSharedUrlToCreate') userSharedUrlToCreate: UserSharedUrlToCreate): Promise<UserSharedUrl | Message> {
        const { email, sharedUrlId, userId } = userSharedUrlToCreate;
        let user;
        if (userId) {
            user = await new UserService().getUserById(userId);
        }
        const sharedUrl = await new SharedUrlService().getSharedUrlById(sharedUrlId);
        if (!sharedUrl) {
